...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    if (authCookie) {
      ...
      const idCookie = cookies.find(c => c.trim().startsWith('auth_user_id='))?.split('=')[1];
      const nameCookie = cookies.find(c => c.trim().startsWith('auth_user_name='))?.split('=')[1];
      const emailCookie = cookies.find(c => c.trim().startsWith('auth_user_email='))?.split('=')[1];
      
      setUser({ 
        id: idCookie || "unknown", 
        name: nameCookie ? decodeURIComponent(nameCookie) : "User", 
        email: emailCookie ? decodeURIComponent(emailCookie) : "", 
        role: roleValue 
      });
    }
  }, []);
  ...
  useIsomorphicLayoutEffect(() => {
    ...
    const cookies = document.cookie.split(';');
    const authCookie = cookies.find(c => c.trim().startsWith('auth_role='));
    const userIdCookie = cookies.find(c => c.trim().startsWith('auth_user_id='));
    const roleValue = authCookie ? authCookie.split('=')[1] : null;

    // If no role cookie OR no user id cookie, they must be on login page
    if (!roleValue || !userIdCookie) {
      // Clear any broken cookies
  ...
  const logout = () => {
    ...
    document.cookie = "auth_user_name=; path=/; expires=Thu, 01 Jan 1970 00:00:00 GMT";
    document.cookie = "auth_user_email=; path=/; expires=Thu, 01 Jan 1970 00:00:00 GMT";
